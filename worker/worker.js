...
async function handleScheduled(env) {
  ...
  for (const { name: key } of keys) {
    ...
    if (!reminder_sent && now >= expiry - FOUR_HOURS && now < expiry) {
      try {
        await sendEmail(email, "Your IPTV Mojo Free Trial is Ready — 24H Access Activated ✓", reminderEmail(name, username, password, m3uUrl), RESEND_KEY, welcome_email_id);
    ...
    if (!followup_sent && now >= expiry) {
      try {
        await sendEmail(email, "Your IPTV Mojo Free Trial is Ready — 24H Access Activated ✓", followupEmail(name), RESEND_KEY, welcome_email_id);
