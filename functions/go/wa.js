...
//
// Note: this site's pages previously had WA_NUMBER='YOUR_WHATSAPP_NUMBER'
// (an unfilled placeholder), so the WhatsApp fallback buttons were broken
// before this fix - they now work AND keep the number out of the page source.
//
...
const BRAND_GREETING = 'Hi IPTVmojo!';

function buildMessage(raw) {
  if (!raw) return BRAND_GREETING;
  let rest = raw.replace(/^(Hallo|Hi|Hello)[^!.,\n]{0,40}[!,.]\s*/i, '').trim();
  if (rest) {
